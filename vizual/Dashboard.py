...
app_mode = st.sidebar.radio("Выберите режим:", ["Heatmap Ошибок", "Оценка Pylint"])

# ==============================================================================
# РЕЖИМ 1: Heatmap 
# ==============================================================================
if app_mode == "Heatmap Ошибок":
    st.title("Лог ошибок регистров ", text_alignment="center")
    ...
    # Создание заглушки log.txt, если нет
    ...
    <style>
        ...
        .main-layout { display: flex; flex-direction: row; justify-content: center; align-items: flex-start; gap: 50px; margin-top: 30px; padding-bottom: 80px; }
        .heatmap-container { display: grid; grid-template-columns: 60px 60px 60px repeat(6, 65px); gap: 8px; align-items: center; position: relative; overflow: visible !important; }
        ...
        .legend-wrapper { display: flex; flex-direction: row; height: 888px; margin-top: 45px; }
    ...
    # Сборка HTML строки
    html_parts = ['<div class="main-layout"><div class="heatmap-container">']
    ...
    html_parts.append('</div>') # End heatmap-container
    ...
    st.markdown(css + full_html, unsafe_allow_html=True)

# ==============================================================================
# РЕЖИМ 2: Оценка Pylint 
# ==============================================================================
elif app_mode == "Оценка Pylint":
    st.title("Анализ кода Pylint", text_alignment="center")

    # --- CSS для увеличения кнопок ---
    st.markdown("""
    <style>
    /* Увеличиваем основную кнопку (Run Pylint) */
    div.stButton > button {
    ...
    /* Увеличиваем заголовок Expander (Показать полный лог) */
    ...
    file_to_check = "final_test.py"
    
    # Создаем файл-заглушку, если нет
    if not os.path.exists(file_to_check):
        with open(file_to_check, "w") as f:
            f.write("import os\n\ndef my_func():\n    x = 1\n    return x\n")

    # Центрирование кнопки с помощью колонок
    # Используем схему: [Пусто] - [Кнопка (2 части)] - [Пусто]
        ...
        with st.spinner("Анализ кода..."):
            try:
                ...
                if score_match:
                    ...
                    # HTML для отображения оценки
                    # margin-top: 40px - отступ от кнопки запуска
                    # margin-bottom: 40px - отступ до кнопки логов
                    html_score = (
                        f'<div style="display: flex; align-items: center; justify-content: center; '
                        f'margin-top: 40px; margin-bottom: 40px; padding: 30px; '
                        f'background-color: #0e1117; border-radius: 15px; border: 1px solid #333;">'
                        f'<div style="width: 30px; height: 30px; background-color: #4ca950; '
                        f'border-radius: 50%; margin-right: 25px; box-shadow: 0 0 15px rgba(76, 169, 80, 0.6);"></div>'
                        f'<div style="font-size: 72px; font-weight: 700; color: #ffffff; '
                        f'font-family: sans-serif; line-height: 1;">{score}/10</div>'
                        f'</div>'
                    )
                    ...
                    st.text(output) # Показываем вывод, если парсинг не удался
                
                # Кнопка логов (Expander)
                # Она автоматически окажется под HTML блоком с отступом margin-bottom: 40px

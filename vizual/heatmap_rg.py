...
# Настройка страницы
st.set_page_config(page_title="Red Heatmap with Dual Legend", layout="wide")
st.title("Лог ошибок в формате Heatmap (Красный)", text_alignment="center")
...
try:
    ...
    st.error(f"Файл {file_path} не найден! Пожалуйста, создайте его в папке с программой.")
    st.stop()

# 2. Парсинг данных из JSON
...
for row in log_data:
    ...
    writes_data.append(row["writes"])
    reads_data.append(row["reads"])
...
# 4. ПАЛИТРЫ (Зеленая - старая, Красная - новая из картинки)
green_colors = [
...
# Красная палитра, воссозданная по приложенному изображению (от бордового до светло-розового)
red_colors = [
...
<style>
    ...
    .cell-wrapper:hover .grid-cell {
        ...
        /* Мягкое свечение изменено на красное */
        box-shadow: 0px 4px 12px rgba(255, 77, 77, 0.45); 
    }
    .grid-cell-empty {
    ...
    /* Цвет выделения текста в подсказке изменен на красный */
    .custom-tooltip b { color: #ff6666; }
...
for y in range(num_registers):
    ...
    for x in range(num_errors):
        ...
        if freq == 0:
            ...
            color_index = int(round(ratio * (len(red_colors) - 1)))
            
            # ТУТ ИСПОЛЬЗУЕТСЯ КРАСНАЯ ПАЛИТРА ДЛЯ ЯЧЕЕК
            hex_color = red_colors[color_index] 
...
# --- ЛЕГЕНДА С 2 ШКАЛАМИ И КРАТНЫМИ 500 ЗНАЧЕНИЯМИ ---
html_content += '<div class="legend-wrapper">'
# Зеленая шкала
html_content += f'<div class="legend-color-bar" style="background: {gradient_green_css};"></div>'
# Красная шкала (с отступом)
html_content += f'<div class="legend-color-bar" style="background: {gradient_red_css}; margin-left: 8px;"></div>'
